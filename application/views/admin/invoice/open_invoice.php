...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Invoice <small>Open Status</small></h2>
      ...
      <div class="x_content">
        <p class="text-muted font-13 m-b-30">
          List open invoice in the system.
        </p>
        <table id="datatable" class="table table-striped table-bordered table-hover">
          ...
          if($invoice){
          	foreach ($invoice as $rs) {  $ttl = 0;  $has_job = 0;
          ?>
            <tr>
              ...
              <td><?php 
                if(isset($arr_ij[$rs->id]) && $arr_ij[$rs->id]){
                  ...
                  $ttl+=round($arr_job_order_ltr[$jrs],2); $has_job=1;
                    ?><span class="badge bg-blue"><a href="<?php echo base_url();?>home/view_orders_content/<?php echo en($jrs,$key);?>" data-toggle="modal" data-target=".bs-example-modal-lg" class="load_modal_details" style="color: #fff;"><?php echo $arr_job_order[$jrs];?></a> | <a href="Javascript:remove_job('<?php echo en($jid,$key);?>');" style="color: #fff;"><span class="fa fa-trash-o"></span></a></span> <?php
                  }
                }else{
                  echo 'no jobs';
                }
              ...
              <td>
                 <div class="btn-group  btn-group-xs"id="tr<?php echo $rs->id;?>">
                  ...
                  <a href="<?php echo base_url();?>invoice/add_billing_address_content/<?php echo en($rs->id,$key);?>/<?php echo en($rs->bill_to,$key);?>" data-toggle="modal" data-target=".bs-example-modal-lg" class="btn btn-warning load_modal_details" type="button" data-placement="bottom" title="Set delivery Address"><i class="fa fa-file-text"></i></a>  

                  <a href="<?php echo base_url();?>invoice/add_additional_fees/<?php echo en($rs->id,$key);?>/<?php echo en($rs->bill_to,$key);?>" data-toggle="modal" data-target=".bs-example-modal-lg" class="btn btn-danger load_modal_details" type="button" data-placement="bottom" title="Add additinal fee"><i class="fa fa-list"></i></a>  
                  
                  <?php if($has_job==1){?>
                  <a href="Javascript:close_invoice('<?php echo en($rs->id,$key);?>');""  class="btn btn-primary" type="button" data-toggle="modal" data-placement="bottom" title="Close Invoice"><i class="fa fa-check-square-o"></i></a>  
                  <?php }?>
              </td>
            </tr> 
           <?php }}?>
...
function close_invoice(str){
  reset(); 

  alertify.confirm("close invoice?", function (e) {
        if (e) {  
            alertify.log("saving...");
            location.href = "<?php echo base_url();?>invoice/set_invoice_to_close/"+str;
        } else {
            alertify.log("cancelled");
        }
    }, "Confirm");
